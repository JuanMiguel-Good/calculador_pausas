import{r as v}from"./index-CKszgeZc.js";function m(e){e.innerHTML=`
<div class="rv-wrap">
...
</div>`,p();const t=e.querySelector("#rvSubmit"),o=e.querySelector("#rvError");function r(i){o.textContent=i,o.style.display="",o.scrollIntoView({behavior:"smooth",block:"nearest"})}t.addEventListener("click",async()=>{o.style.display="none";const i=e.querySelector("#rvName").value.trim(),a=e.querySelector("#rvRuc").value.trim(),s=e.querySelector("#rvAdminName").value.trim(),n=e.querySelector("#rvAdminDni").value.trim(),l=e.querySelector("#rvEmail").value.trim();if(!i)return r("Ingresa el nombre de la empresa.");if(!/^\d{11}$/.test(a))return r("El RUC debe tener 11 dígitos.");if(!s)return r("Ingresa el nombre del administrador.");if(!/^\d{8}$/.test(n))return r("El DNI debe tener 8 dígitos.");if(!l.includes("@"))return r("Ingresa un correo electrónico válido.");t.disabled=!0,t.textContent="Enviando…";try{await v({companyName:i,ruc:a,contactEmail:l,adminName:s,adminDni:n}),e.querySelector("#rvForm").style.display="none",e.querySelector("#rvSuccess").style.display=""}catch(c){const d=c.message||"";d.includes("already registered")?r("Este correo ya está registrado. ¿Quieres iniciar sesión?"):r(d||"Error al enviar la solicitud. Inténtalo de nuevo.")}finally{t.disabled=!1,t.textContent="Enviar solicitud"}})}function p(){if(document.getElementById("rvStyles"))return;const e=document.createElement("style");e.id="rvStyles",e.textContent=`
  ...
  `,document.head.appendChild(e)}export{m as renderRegister};
